...
SymbolQuad getIconQuad(const PositionedIcon& shapedIcon,
    ...
    const float expandX = (iconWidth * (static_cast<float>(image.textureRect.w) + 2.0f * border) / static_cast<float>(image.textureRect.w) - iconWidth) / 2.0f;
    ...
    const float expandY = (iconHeight * (static_cast<float>(image.textureRect.h) + 2.0f * border) / static_cast<float>(image.textureRect.h) - iconHeight) / 2.0f;
